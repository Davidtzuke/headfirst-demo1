...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!copyShader || !clearShader || !splatShader || !advectionShader || 
        ...
        !bloomPrefilterShader || !bloomBlurShader || !bloomFinalShader) return;
    ...
    let advectionProgram = new Material(baseVertexShader, advectionShaderSource);
    ...
    const { BLOOM_ITERATIONS = 8, BLOOM_RESOLUTION = 256, BLOOM_INTENSITY = 0.8, BLOOM_THRESHOLD = 0.6, BLOOM_SOFT_KNEE = 0.7 } = config as any;
    ...
    function render(target: any) {
      ...
      if (config.SHADING) {
        let brightness = config.BLOOM ? 0.5 : 1.0;
      }
    ...
    canvas.addEventListener("touchstart", (e) => {
      ...
      while (pointers.length < touches.length) pointers.push(new (pointerPrototype as any)());
    ...
    canvas.addEventListener("touchmove", (e) => {
      ...
      const touches = e.targetTouches;
      for (let i = 0; i < touches.length; i++) {
        let pointer = getPointer(touches[i].identifier);
        if (!pointer) continue;
        let posX = scaleByPixelRatio(touches[i].pageX);
        let posY = scaleByPixelRatio(touches[i].pageY);
        updatePointerMoveData(pointer, posX, posY);
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if (e.key === " ") splatStack.push(parseInt((Math.random() * 20 + 5).toString()));
    });
